...
module Dram_wrapper (
    ...
    assign delay_done = (state_dram == READ)? delay_cnt == 3'd5 : delay_cnt[2];
    ...
    assign slave.RLAST = ((len_r == len_cnt) & R_hs_done);
    ...
    always_comb begin
        case (state_dram)
            ...
            ACT: begin
                if(delay_done) state_next = (op_r)? WRITE : READ;
                else state_next = ACT;
            end
            READ: begin
    ...
    always_comb begin
        case (state_dram)
            ...
            ACT: begin
                CSn = CSn_ENB;
                CASn = CAS_DIS;
                RASn = (delay_cnt == 3'b0)? RAS_ENB : RAS_DIS;
                WEn = {4{WEn_DIS}};
                A = row;
                D = slave.WDATA;
            end
    ...
    always_ff @( posedge clk) begin
        if(rst)begin
            ...
            WDATA_r <= (state_dram == ACT)? slave.WDATA : WDATA_r;
    ...
    always_ff @( posedge clk) begin 
        if(rst)begin
            ...
            delay_cnt <= ((delay_done) || (state_dram == IDLE))? 3'd0 : (delay_cnt + 3'd1); //***naughty***
            len_cnt <= ((len_cnt == len_r) & R_hs_done) ?`AXI_LEN_BITS'b0 : (R_hs_done)? (len_cnt + `AXI_LEN_BITS'b1) : len_cnt; //
        end
    end
